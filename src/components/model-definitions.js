  ...
  const getRefsFromTag = (tag) => {
    const result = Object.values(data.paths).flatMap(p => Object.values(p)).filter(entryPoint => entryPoint.tags && entryPoint.tags.includes(tag))
      .flatMap(entryPoint => {
        ...
        const requestSchemas = entryPoint.parameters.filter(param => param.in === 'body').filter(response => response.schema && response.schema.$ref).map(response => response.schema.$ref)
